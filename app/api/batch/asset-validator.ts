...
export class AssetValidator {
  ...
  static validateInputs(): boolean {
    ...
    const requiredFiles = [
      { path: AssetPaths.exportZip, name: 'Export zip file' },
    ];
  ...
  static validateDatabase(databasePath: string): boolean {
    try {
      ...
      const requiredProps = ['elements', 'buildMenuCategories', 'buildMenuItems', 'uiSprites', 'spriteModifiers', 'buildings'];
      ...
      AssetLogger.info(`✓ Database structure valid (${database.elements.length} elements, ${database.buildings.length} buildings)`);
      return true;
      
    } catch (error) {
      AssetLogger.error(`Database validation failed: ${error instanceof Error ? error.message : 'Unknown error'}`, error instanceof Error ? error : undefined);
      return false;
    }
  }
  ...
  static validateDiskSpace(): boolean {
    try {
      ...
      AssetLogger.error(`Cannot access project directory: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return false;
    }
  }
  ...
  static safeWriteFile(filePath: string, data: string | Buffer): boolean {
    try {
      ...
      AssetLogger.error(`Failed to write file ${filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`, error instanceof Error ? error : undefined);
      return false;
    }
  }
  ...
  static safeCopyFile(source: string, destination: string): boolean {
    try {
      ...
      AssetLogger.error(`Failed to copy file ${source} → ${destination}: ${error instanceof Error ? error.message : 'Unknown error'}`, error instanceof Error ? error : undefined);
      return false;
    }
  }
  ...
  static safeRemoveDirectory(dirPath: string): boolean {
    try {
      ...
      AssetLogger.error(`Failed to remove directory ${dirPath}: ${error instanceof Error ? error.message : 'Unknown error'}`, error instanceof Error ? error : undefined);
      return false;
    }
  }
  ...
  static validateImageFile(filePath: string): boolean {
    try {
      ...
      AssetLogger.error(`Image validation failed for ${filePath}: ${error instanceof Error ? error.message : 'Unknown error'}`, error instanceof Error ? error : undefined);
      return false;
    }
  }
    ...
    for (const dir of tempDirs) {
      if (fs.existsSync(dir)) {
        try {
          ...
          AssetLogger.warn(`Could not clean up ${dir}: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
      }
    }
  }
