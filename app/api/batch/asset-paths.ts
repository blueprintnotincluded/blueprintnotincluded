...
export class AssetPaths {
  ...
  // Database files
  //
  // Pipeline overview (OniExtract2024 / import:2024):
  //   export/database/  (game export, source of truth)
  //     → npm run import:2024 → assets/database/database-2024.json (committed)
  //                           → frontend/src/assets/database/database-2024.json (committed)
  //
  // Who reads what at runtime:
  //   Angular app    → frontend/src/assets/database/database-2024.zip
  //                    (gitignored; regenerated by frontend prebuild/prestart from the committed JSON)
  //   Backend server → assets/database/database-2024.json

  /** Backend server reads this directly at startup. */
  static get databaseJson(): string {
    return this.absolute('assets/database/database-2024.json');
  }
  /** Zipped from assets/database/database-2024.json. Canonical source for the Angular frontend. */
  static get databaseZip(): string {
    return this.absolute('assets/database/database-2024.zip');
  }
  ...
  /** Committed JSON; frontend prebuild/prestart zips this into frontendDatabaseZip. */
  static get frontendDatabaseJson(): string {
    return this.absolute('frontend/src/assets/database/database-2024.json');
  }
  /** Gitignored zip regenerated at build/serve time. The Angular app fetches this at runtime. */
  static get frontendDatabaseZip(): string {
    return this.absolute('frontend/src/assets/database/database-2024.zip');
  }
