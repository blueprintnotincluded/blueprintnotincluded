...
`;
    ...
    async validateTemplateCompliance(content, templateType) {
        const violations = [];
    ...
    async validateStyleGuide(content) {
        const violations = [];
        ...
        if (listLines.length > 0) {
            ...
            if (uniqueMarkers.length > 1) {
                const firstInconsistentIndex = listLines.findIndex(item => item.line.trim()[0] !== uniqueMarkers[0]);
    ...
    async generateTemplateSuggestions(content, templateType) {
        const requiredSections = this.getRequiredSectionsForCompliance(templateType);
        const existingSections = this.extractExistingSections(content);
        const missingSections = requiredSections.filter(section => !existingSections.some(existing => existing.toLowerCase().includes(section.replace('## ', '').toLowerCase())));
        const corrections = [];
    ...
    async autoCorrectFormatting(content) {
    ...
    async validateDocumentStructure(content, templateType) {
    ...
    getRequiredSectionsForCompliance(templateType) {
        const sectionMap = {
