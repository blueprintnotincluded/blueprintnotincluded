...
var MenubarSub = /** @class */ (function () {
  ...
  MenubarSub.prototype.onItemMenuClick = function (event, item, menuitem) {
    if (!this.autoDisplay) {
      ...
      this.activeItem = this.activeMenu
        ? this.activeMenu.isEqualNode(item)
          ? null
          : item
        : item;
      ...
      if (nextElement) {
        ...
        if (this.root) {
          sublist.style.top =
            domhandler_1.DomHandler.getOuterHeight(item.children[0]) + "px";
          sublist.style.left = "0px";
        } else {
          sublist.style.top = "0px";
          sublist.style.left =
            domhandler_1.DomHandler.getOuterWidth(item.children[0]) + "px";
        }
      }
      this.menuClick = true;
      this.menuHoverActive = this.activeMenu
        ? !this.activeMenu.isEqualNode(item)
        : true;
  ...
  MenubarSub.prototype.onItemMouseEnter = function (event, item, menuitem) {
    if (
      this.autoDisplay ||
      (!this.autoDisplay && this.root && this.menuHoverActive)
    ) {
      if (menuitem.disabled) {
        return;
      }
      if (
        (this.activeItem && !this.activeItem.isEqualNode(item)) ||
        !this.activeItem
      ) {
