...
describe('Image Assets Validation', () => {
  ...
  describe('Frontend Assets (Current State)', () => {
    it('should have frontend images directory', () => {
      ...
      expect(fs.statSync(frontendImagePath).isDirectory(), 'Frontend images path should be directory').to.be.true;
    });
  ...
  describe('Texture Atlases', () => {
    it('should have repack texture files in frontend', () => {
      const frontendRepackFiles = fs.readdirSync(frontendImagePath)
        .filter(f => f.startsWith('repack_') && f.endsWith('.png'));
      
      expect(frontendRepackFiles.length, 'Frontend should have repack texture atlas files').to.be.greaterThan(0);
    });
    ...
    it('should have backend repack textures if assets processed', () => {
      ...
      const backendRepackFiles = fs.readdirSync(assetsImagePath)
        .filter(f => f.startsWith('repack_') && f.endsWith('.png'));
      ...
      if (backendRepackFiles.length > 0) {
        expect(frontendRepackFiles.length, 'Frontend should have repack textures').to.be.greaterThan(0);
        
        // Check a few files exist in both places
        const checkFiles = backendRepackFiles.slice(0, 3);
        checkFiles.forEach(file => {
          expect(frontendRepackFiles.includes(file), 
            `Frontend should have corresponding ${file}`).to.be.true;
        });
      }
    });
  });
  ...
  describe('Group and White Variant Images', () => {
    it('should have group sprite images in frontend if they exist', () => {
      const frontendGroupFiles = fs.readdirSync(frontendImagePath)
      ...
      if (fs.existsSync(assetsImagePath)) {
        const backendGroupFiles = fs.readdirSync(assetsImagePath)
          .filter(f => f.includes('_group_sprite') && f.endsWith('.png'));
    ...
    it('should have white variant images in frontend', () => {
      const frontendWhiteFiles = fs.readdirSync(frontendImagePath)
        .filter(f => f.includes('_white') && f.endsWith('.png'));
      
      expect(frontendWhiteFiles.length, 'Should have white variant images in frontend').to.be.greaterThan(0);
  ...
  describe('File Size Validation', () => {
    ...
    it('should have reasonable file count in frontend assets', () => {
      ...
      expect(allPngFiles.length, 'Should not have excessive frontend PNG assets').to.be.lessThan(10000);
